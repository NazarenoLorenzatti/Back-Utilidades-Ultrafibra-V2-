...
import net.ultrafibra.utilidades.dao.iVariableOidDao;
...
import net.ultrafibra.utilidades.service.iVariableOidService;
...
public class VariableOidServiceImpl implements iVariableOidService {
    ...
    public ResponseEntity<VariableOidResponseRest> guardarVariable(VariableOid variable) {
        ...
        try {
            if (variable != null) {
                Optional<OidClass> oidOptional = oidDao.findById(variable.getOid().getIdOid());
                if (oidOptional.isPresent()) {
                    variable.setOid(oidOptional.get());
                    ...
                    respuesta.setMetadata("Respuesta nok", "-1", "No se pudo guardar la varible, por que no se encontro el OID asociado");
    ...
    public VariableOid obtenerPorId(Long id) {
        VariableOid variableRet = null;
        try {
            if (id != null) {
                Optional<VariableOid> variableOptional = variableDao.findById(id);
                if (variableOptional.isPresent()) {
                    variableRet = variableOptional.get();
                }
            }
        }catch (Exception e) {
            e.getStackTrace();
